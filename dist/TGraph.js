(function (factory) {
  ...
  class Graph {
      ...
      init({height,width,wrapper,nodes,links}){
          ...
          select(wrapper).attr("style","position:relative");
      ...
      on(eventType, callback) {
          const d3Canvas = select("#shadow-canvas");
      ...
      $data({ nodes, links }) {
          ...
          // 布局
          // d3力导布局 + tick
          this.simulation = simulation(nodes).alpha(1)
              .alphaDecay(0.15)
              .force("link", link(links).id((d) => d.id).distance((d) => 30).iterations(10))
              .force("charge", manyBody()) // 多体布局：全局引力或斥力，第一个参数是这个布局的名字，删除时可以用到
              .force("collide", collide().radius((node) => node.symbolSize))
              .force("x", x(this.width / 2)) // x,y 位置布局
              .force("y", y(this.height / 2))
